...
function getUsersFromDatabase(array) {
  const users = Array.from(array, function (element) {
    return {
      id: element.id,
  ...
  return users;
  // Використовуємо метод `Array.from` для створення масиву користувач ів зі списку, елементи якого це об'єкти які міститять
...
function checkArray(obj) {
  if (typeof obj !== "null" && typeof obj !== "undefined") {
    ...
    return "Вхідний об'єкт null або undefined.";
  }
...
function copyAndSwapElements(arr, target, start, end) {
  if (arr.length >= target && arr.length >= start) {
    return arr.copyWithin(target, start, end);
  } else {
    return "Indexes are not allowed";
  }
...
function customFill(arr, value, start = 0, end = arr.length) {
  if (Array.isArray(arr) && start && end >= 0 && start && end <= arr.length) {
